...
interface GlowCardProps {
  children?: ReactNode; // Made optional to support zero-children demos
  className?: string;
...
const GlowCard = forwardRef<HTMLDivElement, GlowCardProps>(({ 
  ...
  // Destructure background to blend with spotlight
  const { background, backgroundImage, ...otherStyles } = style as any;
  ...
  const getInlineStyles = () => {
    const spotlightGradient = `radial-gradient(
    ...
    let combinedBgImage = spotlightGradient;
    if (backgroundImage) {
      combinedBgImage = `${spotlightGradient}, ${backgroundImage}`;
    } else if (background) {
      combinedBgImage = `${spotlightGradient}, ${background}`;
    }

    const baseStyles: React.CSSProperties = {
      '--base': base,
      '--spread': spread,
      '--radius': '24', // Heavily rounded corners (24px border radius)
      '--border': '1',  // Subtle border: 1px solid
      ...
      backgroundImage: combinedBgImage,
