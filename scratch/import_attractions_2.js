...
async function run() {
  console.log("--- Starting Attractions Data Import 2 ---");
  try {
    const dbCities = await fetchAllCities();
    console.log(`Loaded ${dbCities.length} cities from Supabase for local mapping.`);

    const attractionsRawPath = 'C:\\Users\\hafiz\\.gemini\\antigravity\\brain\\cfde45ab-22d0-4261-bac1-e4c10f2a746d\\scratch\\attractions_raw_2.txt';
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!line) continue;
      if (line.includes('____')) continue;

      if (!line.includes(':') && !line.includes('•') && !line.includes('-') && !line.includes('–')) {
        const potentialCountry = line.trim();
        if (potentialCountry.toLowerCase().includes('small territories') || potentialCountry.toLowerCase().includes('islands from your list')) {
          currentCountry = 'dynamic';
        } else {
          currentCountry = potentialCountry;
        }
        continue;
      }
      ...
      if (line.includes('•') || line.startsWith('-') || line.startsWith('*')) {
        ...
        if (cleanLine.toLowerCase().startsWith('uae (already covered')) {
        ...
        const prefix = cleanLine.substring(0, colonIndex).trim();
        const suffix = cleanLine.substring(colonIndex + 1).trim();

        let city = prefix;
        let country = currentCountry;

        const dashMatch = prefix.split(/[–-]/);
        if (dashMatch.length > 1) {
          country = dashMatch[0].trim();
          city = dashMatch.slice(1).join('–').trim();
        }

        if (city.toLowerCase().includes('antarctica')) {
          country = 'Antarctica';
          city = 'Argentine Antarctica';
        }

        let attractions = [];
        if (/\b1\.\s+/.test(suffix)) {
          const splitRegex = /\b\d+\.\s+/;
          attractions = suffix.split(splitRegex).map(a => a.trim()).filter(Boolean);
        } else {
          attractions = suffix.split(',').map(a => a.trim()).filter(Boolean);
        }
    ...
    console.log(`Parsed ${parsedEntries.length} city-entries.`);

    const records = [];
    let attractionIdSeq = 1530; // Start after the first import sequence
    const usedSlugs = new Set();

    for (const entry of parsedEntries) {
      const normalizedCity = normalize(entry.city);
      let normalizedCountry = normalize(entry.country);

      if (normalizedCountry === 'uae') {
        normalizedCountry = 'unitedarabemirates';
      } else if (normalizedCountry === 'bonaire') {
        normalizedCountry = 'bonairesinteustatiusandsaba';
      }
      ...
      if (!matchedCity) {
        console.warn(`WARNING: Unmapped city "${entry.city}" in country "${entry.country}". Skipping.`);
        continue;
      }

      for (const attr of entry.attractions) {
    ...
    console.log(`Parsed total of ${records.length} new attraction records.`);
    ...
    console.log("✓ Attractions import 2 completed successfully!");
  } catch (err) {
    console.error("FATAL ERROR: Import 2 failed:", err);
    process.exit(1);
  }
}
