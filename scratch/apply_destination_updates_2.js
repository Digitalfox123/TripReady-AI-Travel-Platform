...
// 1. OVERHAUL isCountryDestination
const oldIsCountry = `  const isCountryDestination = (dest) => {
    const cId = dest.id.toLowerCase();
    return cId === 'switzerland' || cId === 'norway' || cId === 'new-zealand' || cId === 'costa-rica' || cId === 'pakistan' || cId === 'spain' || cId === 'thailand' || cId === 'turkey' || cId === 'egypt' || cId === 'italy' || cId === 'france' || cId === 'united-kingdom' || cId === 'saudi-arabia';
  };`;

const newIsCountry = `  const isCountryDestination = (dest) => {
    const cId = dest.id.toLowerCase();
...
if (content.includes(oldIsCountry)) {
  content = content.replace(oldIsCountry, newIsCountry);
  console.log("Successfully updated isCountryDestination");
} else {
  console.error("COULD NOT FIND oldIsCountry BLOCK");
}

// 2. OVERHAUL getCountryRegions
// Let's locate the old getCountryRegions block and replace it up to:
//   const getCountryRegions = (dest) => { ... return [ { name: `Capital Region of ${dest.name}` ... } ]; };
// Since getCountryRegions is very large, let's write a regex or targeted string replacement.
// Let's print out what is around it to replace precisely.
// In the first view, we saw getCountryRegions starts at line 1305 and the return statement of getCountryRegions is on line 1339.
// Let's match it using index of getCountryRegions and index of the return statement or similar.
// Wait! Let's just find the start of getCountryRegions: `const getCountryRegions = (dest) => {` and the end:
// Let's find the closing brace that matches it.
// Alternatively, let's use a very specific string replace.

const oldGetCountryRegions = `  const getCountryRegions = (dest) => {
    const cId = dest.id.toLowerCase();
    if (cId === 'switzerland') {
      return [
...
const newGetCountryRegions = `  const getCountryRegions = (dest) => {
    const cId = dest.id.toLowerCase();
    if (cId === 'switzerland') {
      return [
        { name: 'Zurich', type: 'Urban Luxury & Clockwork', time: 'Base City', activity: 'Explore the historic Bahnhofstrasse, lakeside plazas, and medieval Altstadt.' },
...
if (content.includes(oldGetCountryRegions)) {
  content = content.replace(oldGetCountryRegions, newGetCountryRegions);
  console.log("Successfully updated getCountryRegions");
} else {
  // Try matching without carriage returns
  const normalizedContent = content.replace(/\r\n/g, '\n');
  const normalizedOld = oldGetCountryRegions.replace(/\r\n/g, '\n');
  const normalizedNew = newGetCountryRegions.replace(/\r\n/g, '\n');
  if (normalizedContent.includes(normalizedOld)) {
    content = normalizedContent.replace(normalizedOld, normalizedNew);
    console.log("Successfully updated getCountryRegions (Normalized)");
  } else {
    console.error("COULD NOT FIND getCountryRegions BLOCK");
  }
}
